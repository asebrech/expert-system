pub mod solver {
    ...
    fn are_requirements_met(
        ...
        let mut last_result =
            match_requirement(first_req, brain, first_req.condition, true, false, depth);
    ...
    fn match_requirement(
        ...
        match condition {
            ...
            Condition::OR => {
                if last_result == true
                    && current_knowledge_truthy.unwrap() == true
                    && same_condition == true
                {
                    return Some(false);
                }
    ...
    pub fn init() {
        ...
        //A
        println!(
            "\nA should be undetermined (true) : {}",
            get_knowledge_state("A", &knowledge_engine, 0).is_none()
        );
